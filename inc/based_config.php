...
// alx change 
if (!defined('GLPI_ExtFiles')) {
    define('GLPI_ExtFiles', dirname(__FILE__, 3));
}
    ...
    $constants = [
      // Constants related to system paths
       // 'GLPI_CONFIG_DIR'      => GLPI_ROOT . '/config', // Path for configuration files (db, security key, ...)
        'GLPI_CONFIG_DIR'      => GLPI_ExtFiles . '/glpi_ExternalDirs_ver_9/config', // Path for configuration files (db, security key, ...)
       // 'GLPI_VAR_DIR'         => GLPI_ROOT . '/files',  // Path for all files
        'GLPI_VAR_DIR'         => GLPI_ExtFiles . '/glpi_ExternalDirs_ver_9/files',  // Path for all files  <-- Alx change
